...
OUTPUT CONTRACT — three buckets. For every one of the 44 candidates below, sort it into exactly one of:
...
CANDIDATE LIST (44 items, root: /Users/lucasabud/Projects/api-drift/rule_test/entanglement_experiment/host):
...
OUTPUT FORMAT — your final response must end with a fenced ```json code block, and every one of the 44 candidates above must appear in exactly one of the three arrays below (matched by file+line -- for a candidate with duplicate_lines, use its first/representative line):
