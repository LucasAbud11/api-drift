...
FIXGEN = os.path.dirname(HERE)
REPO_ROOT = os.path.dirname(os.path.dirname(FIXGEN))
REPOS_SRC = os.path.join(REPO_ROOT, "repos")
STUB = os.path.join(HERE, "mcp_v2_stub")
...
def make_scratch(run_n):
    scratch = os.path.join(SCRATCH_BASE, f"fixgen_verify_run{run_n}")
    if os.path.exists(scratch):
        shutil.rmtree(scratch)
    os.makedirs(scratch)
    for repo in TARGETB_SMALL_REPOS:
        shutil.copytree(os.path.join(REPOS_SRC, repo), os.path.join(scratch, repo))
    return scratch
...
def run_qainsights_suite(scratch):
    repo_dir = os.path.join(scratch, "QAInsights_jmeter-mcp-server")
    env = dict(os.environ)
    env["PYTHONPATH"] = "."
    proc = subprocess.run(
...
def verify_run(run_n):
    run_path = os.path.join(FIXGEN, "runs", f"run{run_n}.json")
